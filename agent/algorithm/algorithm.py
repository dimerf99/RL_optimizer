...
import statistics
from tedeous.RL_repo.DQN_classes import DQN_optim, DQN_params
...
class DQNAgent:
    ...
    def get_random_action(self):
        ...
        for key in optim_class_dict:
            if key == 'epochs':
                epochs_class = random.randint(0, len(optim_class_dict['epochs']) - 1)
            else:
            ...
            if sample > eps_threshold:
                with torch.no_grad():
                    ...
                    for key in param_dict:
                        if key == 'epochs':
                            epochs_class = torch.argmax(param_dict[key])
                        else:
                            param_class[key] = torch.argmax(param_dict[key])
            else:
